...
public class Result<T> where T : class
{
    ...
    public Result(bool error, string message, int statusCode, T? value = null)
    {
        Error = error;
        Message = message;
    ...
    {
        return new Result<T>(false, message, statusCode, value);
    }
...
public class Result
{
    ...
    public Result(bool error, string message, int statusCode, object? value = null)
    {
        Error = error;
        Message = message;
        StatusCode = statusCode;
    ...
    public static Result Success(object? value, string message = "Operación exitosa", int statusCode = 400)
         => new Result(false, message, statusCode, value);
